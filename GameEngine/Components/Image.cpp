...
void Image::RenderComponent() const
{
	...
	const glm::vec2 transformPosition{ pOwnerTransform->GetWorldPosition().x, 
		pOwnerTransform->GetWorldPosition().y };

	const glm::vec2 texSize{ m_pTexture->GetSize() };
	const glm::vec2 scaledSize{ texSize * glm::vec2{pOwnerTransform->GetWorldScale().x, 
		pOwnerTransform->GetWorldScale().y}};
