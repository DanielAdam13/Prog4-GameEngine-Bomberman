...
# Game-Engine Architectural Choices
...
There are generic and reusable commands in the game engine such as Move. The more specific commands should be added from game-side. 
The Game Engine exposes an interesting Command wrapper - Conditional Command. This class basically stores ANY Command and a std::function<bool()> Condition, meaning that it is saying: "A specific command can ever think of executing AFTER evaluating the condition".
This should be used as a generic/base condition because it is the top-most level of evaluation for executing a command. A SPECIFIC condition should happen in its SPECIFIC Command's Execute method itself.
